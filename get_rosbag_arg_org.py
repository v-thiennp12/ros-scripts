...
def main(argv):
    ...
    #save_path = os.path.basename(str(sys.argv[1]))
    ...
    #videoWriter_h264 = cv2.VideoWriter(tmp_filename, cv2.cv.FOURCC(*'h264'), 20, (1920,1208))
    ...
    for topic, msg, t in bag.read_messages(topics=['/camera/tricam/mid/image_raw/compressed']):
        try:
            ...
            #cv2.imwrite("frame_id_{}.jpeg".format(count), cv_img)
    ...
    #videoWriter_h264 = None
    os.system('%s -vcodec libx264 -crf 28 %s' % (tmp_filename, save_vid))
    #os.system('%s -vcodec libx264 %s' % (tmp_filename, save_vid))
